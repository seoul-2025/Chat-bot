...
class WebSocketService {
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        const processedHistory = historyToUse.map((msg) => {
          ...
          // 이전 메시지에서 파일 내용 제거 (새 메시지와 중복 방지)
          if (msg.type === "user" && content.includes("--- 파일:")) {
            // 파일 내용 부분을 제거하고 사용자 텍스트만 유지
            const fileStartIndex = content.indexOf("\n\n--- 파일:");
            if (fileStartIndex !== -1) {
              content = content.substring(0, fileStartIndex).trim();
            }
          }
      ...
      await claudeService.streamChat(
        ...
        (error) => {
          ...
          this.messageHandlers.forEach((handler) => {
            handler({
              type: "error",
              message: `Claude API 오류: ${error.message}`
            });
          });
        }
      );
  ...
  generateMockResponse(message, engineType) {
    ...
    // 모의 응답 텍스트 - 파일 내용 제거하고 사용자 텍스트만 추출
...
1. .env.local 파일에 VITE_CLAUDE_API_KEY 설정
