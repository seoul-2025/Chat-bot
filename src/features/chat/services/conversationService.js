...
class ConversationService {
  ...
  async saveConversation(conversationData) {
    try {
      // 개발 환경에서는 localStorage만 사용
      if (import.meta.env.DEV) {
        console.log("🔧 개발 모드: localStorage에 대화 저장");
        this.saveToLocalStorage(conversationData);
        return { success: true, conversationId: conversationData.conversationId };
      }
  ...
  async listConversations(engineType = null) {
    // 개발 환경에서는 localStorage만 사용
    if (import.meta.env.DEV) {
      console.log("🔧 개발 모드: localStorage에서 대화 목록 조회");
  ...
  async getConversation(conversationId) {
    // 개발 환경에서는 localStorage만 사용
    if (import.meta.env.DEV) {
      console.log("🔧 개발 모드: localStorage에서 대화 조회");
