      ...
      if ((message.trim() || uploadedFiles.length > 0) && !isLoading) {
        ...
        if (!onStartChat && isConnected) {
          ...
          try {
            ...
            await sendChatMessage(fullMessage, engineType, [], null, null, selectedModel);
              ...
              <div className="w-full font-large break-words transition-opacity duration-200 min-h-[2rem]">
                <textarea
                  ...
                  placeholder={
                    isConnected
                      ? engineType === "11"
                        ? "기업 보도자료를 붙여넣거나 PDF파일을 첨부하세요. (AI 분석 후 맞춤 옵션 제시 → 기사 완성)"
                        : "정부/공공기관 보도자료를 붙여넣거나 PDF 파일을 첨부하세요. (AI 분석 후 맞춤 옵션 제시 → 기사 완성)"
                      : "서버 연결 중..."
                  }
              ...
              <div className="relative shrink-0 model-dropdown">
                  ...
                  >
                    <span className="truncate max-w-20">
                      {(() => {
                        const models = JSON.parse(import.meta.env.VITE_CLAUDE_MODELS || '[]');
                        const model = models.find(m => m.id === selectedModel);
                        return model ? model.name : 'Opus 4.5';
                      })()}
                    </span>
                    <svg
                  ...
                  {isModelDropdownOpen && (
                    ...
                    >
                      <div className="p-1">
                        {JSON.parse(import.meta.env.VITE_CLAUDE_MODELS || '[]').map((model) => (
                          <button
                            key={model.id}
                            onClick={() => {
