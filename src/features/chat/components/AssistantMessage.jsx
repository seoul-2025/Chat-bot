...
const AssistantMessage = React.memo(({ content, timestamp, messageId, sources, sourceTitle }) => {
  ...
  const formatContent = () => {
    if (hasValidTitles) {
      ...
      // 첫 번째 줄이 제목처럼 보이면 마크다운 제목으로 변환
      if (textContent) {
        const lines = textContent.split('\n');
        if (lines.length > 0 && lines[0].trim()) {
          const firstLine = lines[0].trim();
          // 첫 번째 줄이 마크다운 제목이 아니고, 제목처럼 보이면 # 추가
          if (!firstLine.startsWith('#') && 
              (firstLine.includes('이슈') || firstLine.includes('주요') || 
               firstLine.includes('핵심') || firstLine.includes('중요') ||
               firstLine.includes('트렌드') || firstLine.includes('변화') ||
               firstLine.endsWith('가지') || firstLine.endsWith('개') ||
               /^\d+/.test(firstLine))) {
            lines[0] = '# ' + firstLine;
      ...
      return (
        <div
          ...
          style={{
            fontFamily: '"Tiempos Text", "Source Serif 4", "Noto Serif KR", serif !important',
          }}
        >
          <ReactMarkdown
  ...
  return (
    <div className="mb-8 last:mb-0 transition-all duration-200">
      {/* Sources Card */}
      <SourceCard title={sourceTitle} sources={sources} />
      ...
      >
        <div
          ...
          style={{
            fontFamily:
              '"Tiempos Text", "Source Serif 4", "Noto Serif KR", serif',
            fontSize: "17px",
            lineHeight: "1rem",
