...
try:
    ...
    prompts_table_name = os.environ.get('PROMPTS_TABLE', 'w1-prompts-v2')
    files_table_name = os.environ.get('FILES_TABLE', 'w1-files')
...
def handle_prompts(method: str, path_params: Dict, body: Dict) -> Dict:
    ...
    if method == 'GET':
        ...
        if engine_type:
            try:
                # DynamoDB 단일 키 사용: promptId만 사용
                # promptId가 엔진 타입과 동일 (예: '11', '22', '33')
        ...
        try:
            ...
            item = {
                'promptId': engine_type,     # 단일 키
                'engineType': engine_type,   # 추가 속성
...
# 임시 우회: PUT_ITEM 사용
                ...
                updated_item['promptId'] = engine_type     # 단일 키
                updated_item['engineType'] = engine_type   # 추가 속성
