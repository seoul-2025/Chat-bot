    ...
    try:
        ...
        # 메시지 전송 액션
            ...
            send_message_to_client(connection_id, {
                'type': 'thinking_status',
                'step': 'preparing',
                'message': '맥락을 파악하고 프롬프트를 준비 중입니다...'
            }, apigateway_client)

            # 4. 사고 과정 3단계: AI 모델 준비
            send_message_to_client(connection_id, {
                'type': 'thinking_status',
                'step': 'ai_ready',
                'message': 'AI가 답변을 생성합니다...'
            }, apigateway_client)
...
def send_message_to_client(connection_id, message, apigateway_client):
    ...
    try:
        ...
        try:
            ...
            connections_table = dynamodb.Table(os.environ.get('WEBSOCKET_TABLE', 'p2-two-websocket-connections-two'))
