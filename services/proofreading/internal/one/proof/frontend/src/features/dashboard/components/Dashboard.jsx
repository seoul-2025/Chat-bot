...
const Dashboard = ({ selectedEngine = "Basic", onBack }) => {
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div className="flex gap-1 sm:gap-2 bg-bg-300/50 rounded-lg p-0.5 sm:p-1">
                {[
                  { id: 'Basic', label: '비즈니스', fullLabel: '비즈니스 모드', path: '/11/dashboard' },
                  { id: 'Pro', label: '종합 뉴스', fullLabel: '종합 뉴스 모드', path: '/22/dashboard' }
                ].map((engine) => (
                  <button
                    key={engine.id}
