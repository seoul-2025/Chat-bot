...
export const useDashboard = (userRole) => {
  ...
  const generateMockData = () => {
    ...
    const engineComparison = [
      { name: 'Basic', value: 65, fill: 'hsl(251, 40.2%, 54.1%)' },
      { name: 'Pro', value: 35, fill: 'hsl(210, 70.9%, 51.6%)' }
    ];
  ...
  const loadUsageData = async () => {
    try {
      const basicUsage = await usageService.getUsagePercentage('Basic');
      const proUsage = await usageService.getUsagePercentage('Pro');
      
      setUsageData({
        T5: {
          percentage: basicUsage || 0,
          used: Math.floor((basicUsage || 0) * 30),
          limit: 3000,
          cost: Math.floor((basicUsage || 0) * 5)
        },
        H8: {
          percentage: proUsage || 0,
          used: Math.floor((proUsage || 0) * 20),
          limit: 2000,
          cost: Math.floor((proUsage || 0) * 8)
        },
        totalCost: Math.floor((basicUsage || 0) * 5) + Math.floor((proUsage || 0) * 8),
        dailyLimit: 100,
        monthlyLimit: 3000
      });
    } catch (error) {
  ...
  const refreshData = async () => {
    ...
    try {
      await Promise.all([
        usageService.fetchUsageFromServer('Basic'),
        usageService.fetchUsageFromServer('Pro')
      ]);
