...
const Sidebar = forwardRef(({ selectedEngine = 'Basic', isOpen = true, onToggle }, ref) => {
  ...
  const loadConversations = async () => {
    try {
      ...
      console.log(`📊 사이드바 대화 목록 (${selectedEngine}):`, {
        ...
        firsbasic: convs.slice(0, 5).map(c => ({
          id: c.conversationId,
          title: c.title,
      ...
      <div className="flex flex-col px-1.5 pt-0.5 gap-px mb-2">
        <button
          onClick={() => {
            ...
            window.location.href = `/${getEnginePathFromName(selectedEngine)}`;
          }}
...
const ConversationItem = ({ 
  ...
  return (
    <li className="relative group">
      <Link
        to={`/${getEnginePathFromName(selectedEngine)}/chat/${conversation.conversationId}`}
        ...
        onClick={() => {
          console.log("🔗 스레드 클릭:", {
            ...
            targetUrl: `/${getEnginePathFromName(selectedEngine)}/chat/${conversation.conversationId}`,
