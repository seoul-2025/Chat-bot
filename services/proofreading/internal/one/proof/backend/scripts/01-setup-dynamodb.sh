...
# 기존 테이블 삭제 (있는 경우)
echo "기존 테이블 삭제 중..."
aws dynamodb delete-table --table-name nx-wt-prf-prompts --region us-east-1 2>/dev/null || true
sleep 5

# 1. Prompts 테이블 생성 (설명과 지침 저장)
echo "Prompts 테이블 생성 중..."
aws dynamodb create-table \
    --table-name nx-wt-prf-prompts \
...
# 테이블이 생성될 때까지 대기
echo "테이블 생성 확인 중..."
aws dynamodb wait table-exists --table-name nx-wt-prf-prompts --region us-east-1
aws dynamodb wait table-exists --table-name nx-wt-prf-files --region us-east-1
...
echo ""
echo "생성된 테이블:"
echo "1. nx-wt-prf-prompts (설명, 지침)"
echo "2. nx-wt-prf-files (파일들)"