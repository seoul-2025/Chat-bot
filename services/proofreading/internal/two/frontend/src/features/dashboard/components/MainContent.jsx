...
import BasicNProGuideSection from "./T5NH8GuideSection";
...
const MainContent = ({
  ...
  // }); // 사용량 상태 추가 - 비활성화
  const usagePercentage = null; // 사용량 기능 비활성화
        ...
        <div className="flex-1 flex flex-col gap-4">
          ...
          >
            ...
            {userRole === "admin" ? (
              ...
              /* Welcome Message with Logo (사용자용) */
              <div
                className="font-display text-text-200 w-full flex items-center justify-center transition-opacity duration-300 ease-in"
                style={{
              ...
              >
                ...
                <div className="select-none" style={{ marginLeft: "4rem" }}>
                  <span className="block sm:inline">
                    {selectedEngine === 'Basic' ? '비즈니스 모드' : '종합 뉴스 모드'} -
                  </span>
                  <span className="block sm:inline sm:ml-1">
            ...
            {userRole === "user" && (
              <BasicNProGuideSection selectedEngine={selectedEngine} />
            )}
          </div>
        </div>
