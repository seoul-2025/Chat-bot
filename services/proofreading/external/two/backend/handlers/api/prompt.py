...
prompts_table = dynamodb.Table('nx-wt-prf-prompts')
files_table = dynamodb.Table('nx-wt-prf-files')
...
def handle_prompts(method: str, path_params: Dict, body: Dict) -> Dict:
    ...
    # path_params가 None일 수 있으므로 안전하게 처리
    safe_path_params = path_params or {}
    
    # promptId와 engineType 둘 다 지원 (API Gateway 호환성)
    engine_type = safe_path_params.get('promptId') or safe_path_params.get('engineType')
...
def handle_files(method: str, path_params: Dict, body: Dict) -> Dict:
    ...
    # promptId와 engineType 둘 다 지원 (API Gateway 호환성)
    engine_type = safe_path_params.get('promptId') or safe_path_params.get('engineType') 
    file_id = safe_path_params.get('fileId')
