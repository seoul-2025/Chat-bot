"""
AWS Bedrock Claude 클라이언트 - 최적화 버전 (Prompt Caching 적용)
...
def _build_cached_system_blocks(system_prompt: str, prompt_data: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    logger.info(f"✅ Prompt caching enabled - system prompt: {len(system_prompt)} chars")
    ...
    try:
        ...
        if enable_caching and prompt_data:
            ...
            body = {
                ...
                "messages": messages,
                # top_p 제거: Claude 4.x는 temperature와 top_p를 동시에 사용할 수 없음
                "top_k": TOP_K
            }
...
class BedrockClientEnhanced:
    ...
    def stream_bedrock(
        ...
        """
        Bedrock 스트리밍 응답 생성 - Prompt Caching 지원
        ...
        try:
            ...
            # 대화 컨텍스트를 user_message에 포함 (캐시 히트율 향상)
            ...
            for chunk in stream_claude_response_enhanced(
                user_message=enhanced_user_message,  # ✅ 컨텍스트가 포함된 메시지
                system_prompt=system_prompt,  # ✅ 정적 프롬프트 (캐싱 가능)
    ...
    def _create_system_prompt_with_context(
        ...
        ⚠️ 중요: 캐시 히트를 위해 대화 컨텍스트를 시스템 프롬프트에 포함하지 않습니다.
        대화 컨텍스트는 user_message에 포함됩니다.
        """
        ...
        # ❌ 대화 컨텍스트를 시스템 프롬프트에 추가하지 않음 (캐시 미스 방지)
    ...
    def _create_user_message_with_context(
        ...
        """
        대화 컨텍스트를 포함한 사용자 메시지 생성

        ✅ 대화 컨텍스트를 user_message에 포함하여 캐시 히트율 향상
        """
