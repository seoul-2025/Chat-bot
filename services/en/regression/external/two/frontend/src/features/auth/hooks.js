    ...
    try {
      ...
      if (result.success) {
        ...
        if (formData.username === ADMIN_EMAIL || result.user.email === ADMIN_EMAIL) {
          console.log('🔐 Admin email matched:', ADMIN_EMAIL);
          userRole = 'admin';
          userPlan = 'premium';
        } else if (COMPANY_DOMAIN && COMPANY_DOMAIN.trim() !== '' && result.user.email?.includes(COMPANY_DOMAIN)) {
          console.log('🔐 Company domain matched:', COMPANY_DOMAIN);
          userRole = 'admin';
          userPlan = 'premium';
        }
      ...
      if (result.success) {
        ...
        if (loginResult.success) {
          ...
          if (formData.username === ADMIN_EMAIL || loginResult.user.email === ADMIN_EMAIL) {
            userRole = 'admin';
            userPlan = 'premium';
          } else if (COMPANY_DOMAIN && COMPANY_DOMAIN.trim() !== '' && loginResult.user.email?.includes(COMPANY_DOMAIN)) {
            userRole = 'admin';
            userPlan = 'premium';
          }
          localStorage.setItem('userRole', userRole);
