...
update_lambda_function() {
    ...
    # 4. 환경변수 업데이트 (통합된 환경변수 설정 - .env 파일 기반)
    # 주의: AWS_REGION은 Lambda 예약 키이므로 설정하지 않음 (자동으로 설정됨)
    echo "   🔧 환경변수 업데이트 중..."
    aws lambda update-function-configuration \
        ...
        --environment "Variables={
            ...
            CLOUDWATCH_NAMESPACE=er1-two,
            LOG_GROUP=/aws/lambda/er1-two,
