...
const FileUploadButton = forwardRef(({ onFileContent, disabled }, ref) => {
  ...
  const processFile = async (file) => {
    ...
    try {
      ...
      if (file.type === 'text/plain' || file.name.endsWith('.txt')) {
        ...
        alert('Unsupported file format. Only TXT and PDF files are allowed.');
        return;
      }
  ...
  const handlePdfFile = async (file) => {
    try {
      ...
      // 추출한 텍스트가 비어있는지 확인
      if (!fullText.trim()) {
        alert('Unable to extract text from PDF. It may be a scanned image PDF.');
        return null;
      }
      ...
      alert('An error occurred while processing the PDF file. Please copy and paste the text directly.');
      return null;
    }
  };
  ...
  return (
    <div className="relative group">
      ...
      <button
        ...
        title={isProcessing ? 'Processing file...' : 'Upload file (TXT, PDF)'}
        style={{
