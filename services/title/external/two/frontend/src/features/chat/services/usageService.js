...
export const getLocalUsageData = () => {
  try {
    ...
    if (data.C7) {
      data.C7.planType = userProfile.currentPlan;
      data.C7.limits = planLimits.C7;
    }
...
export const getAllUsageStats = () => {
  ...
  return {
    ...
    engines: {
      ...
      C7: getUsageSummary("C7"),
    },
    total: {
      tokens: {
        total:
          (usageData.T5?.tokens.total || 0) + (usageData.C7?.tokens.total || 0),
        input:
          (usageData.T5?.tokens.input || 0) + (usageData.C7?.tokens.input || 0),
        output:
          (usageData.T5?.tokens.output || 0) +
          (usageData.C7?.tokens.output || 0),
      },
      characters: {
        input:
          (usageData.T5?.characters.input || 0) +
          (usageData.C7?.characters.input || 0),
        output:
          (usageData.T5?.characters.output || 0) +
          (usageData.C7?.characters.output || 0),
      },
      messages:
        (usageData.T5?.messageCount || 0) + (usageData.C7?.messageCount || 0),
    },
  };
};
...
export const getAllUsageData = async () => {
  try {
    ...
    // T5와 C7 각각 호출
    const [t5Response, c7Response] = await Promise.all([
      fetch(
      ...
      fetch(
        `${USAGE_API_BASE_URL}/usage/${encodeURIComponent(user.userId)}/C7`,
        {
          method: "GET",
          headers: getAuthHeaders(),
        }
    ...
    const c7Result = await c7Response.json();

    console.log("📊 T5 응답:", t5Result);
    console.log("📊 C7 응답:", c7Result);

    // 엔진별 데이터 정리
    const t5Data = t5Result.success ? t5Result.data : null;
    const c7Data = c7Result.success ? c7Result.data : null;
    ...
    return {
      ...
      C7: {
        monthlyTokensUsed: c7Data?.totalTokens || 0,
        inputTokens: c7Data?.inputTokens || 0,
        outputTokens: c7Data?.outputTokens || 0,
        charactersProcessed: 0, // 간단화
        messageCount: c7Data?.messageCount || 0,
        lastUsedAt: c7Data?.lastUsedAt,
        limits: PLAN_LIMITS[user.plan]?.C7 || PLAN_LIMITS.free.C7,
      },
    };
  } catch (error) {
