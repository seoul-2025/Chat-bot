...
export const useDashboard = (userRole) => {
  ...
  const [usageData, setUsageData] = useState({
    C1: { percentage: 0, used: 0, limit: 0, cost: 0 },
    C2: { percentage: 0, used: 0, limit: 0, cost: 0 },
    totalCost: 0,
    dailyLimit: 100,
    monthlyLimit: 3000
  });
  ...
  const generateMockData = () => {
    ...
    const daily = Array.from({ length: 24 }, (_, i) => ({
      name: `${i}시`,
      C1: Math.floor(Math.random() * 100) + 20,
      C2: Math.floor(Math.random() * 80) + 10,
    ...
    const weekly = ['월', '화', '수', '목', '금', '토', '일'].map(day => ({
      name: day,
      C1: Math.floor(Math.random() * 500) + 100,
      C2: Math.floor(Math.random() * 400) + 80,
    ...
    const monthly = Array.from({ length: 30 }, (_, i) => ({
      name: `${i + 1}일`,
      C1: Math.floor(Math.random() * 200) + 50,
      C2: Math.floor(Math.random() * 150) + 40,
      cost: Math.floor(Math.random() * 100) + 20
    }));

    // 엔진 비교 데이터
    const engineComparison = [
      { name: 'C1', value: 65, fill: 'hsl(251, 40.2%, 54.1%)' },
      { name: 'C2', value: 35, fill: 'hsl(210, 70.9%, 51.6%)' }
    ];
    ...
    // 성공률 데이터
    const successRate = [
      { subject: '속도', C1: 95, C2: 80, fullMark: 100 },
      { subject: '정확도', C1: 88, C2: 92, fullMark: 100 },
      { subject: '창의성', C1: 75, C2: 90, fullMark: 100 },
      { subject: '안정성', C1: 92, C2: 85, fullMark: 100 },
      { subject: '비용효율', C1: 90, C2: 70, fullMark: 100 }
    ];
  ...
  const loadUsageData = async () => {
    try {
      const t5Usage = await usageService.getUsagePercentage('C1');
      const c7Usage = await usageService.getUsagePercentage('C2');
  ...
  const refreshData = async () => {
    ...
    try {
      await Promise.all([
        usageService.fetchUsageFromServer('C1'),
        usageService.fetchUsageFromServer('C2')
      ]);
