  ...
  async connect() {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let wsUrl = import.meta.env.VITE_WS_URL || 'wss://ebqodb8ax9.execute-api.us-east-1.amazonaws.com/production';
        ...
        if (messageChunks.length > 1) {
          ...
          messageChunks.forEach((chunk, index) => {
            const payload = {
              action: "sendMessage",
              message: chunk,
              engineType: this.mapEngineTypeForAPI(engineType), // 백엔드용 엔진 타입으로 변환
