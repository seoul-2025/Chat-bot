...
const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || 'https://t75vorhge1.execute-api.us-east-1.amazonaws.com/prod';

// Usage API 전용 엔드포인트 (DynamoDB 연동)
const USAGE_API_BASE_URL = import.meta.env.VITE_USAGE_API_URL || '';

// 프론트엔드 엔진 타입을 백엔드 엔진 타입으로 매핑
const mapEngineTypeForAPI = (frontendEngineType) => {
  const engineMapping = {
    '11': 'C1',
    '22': 'C2'
  };
...
const mapEngineTypeFromAPI = (backendEngineType) => {
  const engineMapping = {
    'C1': '11',
    'C2': '22'
  };
...
export const updateLocalUsage = async (engineType, inputText, outputText) => {
  try {
    ...
    const response = await fetch(`${USAGE_API_BASE_URL}/usage/update`, {
      ...
      body: JSON.stringify({
        userId: user.userId,
        engineType: mapEngineTypeForAPI(engineType), // 백엔드용 엔진 타입으로 변환
        inputText: inputText || "",
...
export const getUsagePercentage = async (engineType, forceRefresh = false) => {
  try {
    ...
    const response = await fetch(
      `${USAGE_API_BASE_URL}/usage/${user.userId}/${mapEngineTypeForAPI(engineType)}`,
      {
        headers: getAuthHeaders(),
      }
    );
...
export const getAllUsageStats = () => {
  ...
  return {
    ...
    total: {
      tokens: {
        total:
          (usageData["11"]?.tokens.total || 0) + (usageData["22"]?.tokens.total || 0),
        input:
          (usageData["11"]?.tokens.input || 0) + (usageData["22"]?.tokens.input || 0),
        output:
          (usageData["11"]?.tokens.output || 0) +
          (usageData["22"]?.tokens.output || 0),
      },
      characters: {
        input:
          (usageData["11"]?.characters.input || 0) +
          (usageData["22"]?.characters.input || 0),
        output:
          (usageData["11"]?.characters.output || 0) +
          (usageData["22"]?.characters.output || 0),
      },
      messages:
        (usageData["11"]?.messageCount || 0) + (usageData["22"]?.messageCount || 0),
    },
  };
};
...
export const getAllUsageData = async () => {
  try {
    ...
    // 11과 22 각각 호출 (API는 백엔드 C1/C2로 매핑)
    const [engine11Response, engine22Response] = await Promise.all([
      fetch(
        `${USAGE_API_BASE_URL}/usage/${encodeURIComponent(user.userId)}/${mapEngineTypeForAPI("11")}`,
        {
          method: "GET",
          headers: getAuthHeaders(),
        }
      ...
      fetch(
        `${USAGE_API_BASE_URL}/usage/${encodeURIComponent(user.userId)}/${mapEngineTypeForAPI("22")}`,
        {
          method: "GET",
          headers: getAuthHeaders(),
        }
    ...
    const engine11Result = await engine11Response.json();
    const engine22Result = await engine22Response.json();

    console.log("📊 11 응답:", engine11Result);
    console.log("📊 22 응답:", engine22Result);

    // 엔진별 데이터 정리
    const engine11Data = engine11Result.success ? engine11Result.data : null;
    const engine22Data = engine22Result.success ? engine22Result.data : null;
    ...
    return {
      ...
      signupDate: new Date().toISOString(),
      "11": {
        monthlyTokensUsed: engine11Data?.totalTokens || 0,
        inputTokens: engine11Data?.inputTokens || 0,
        outputTokens: engine11Data?.outputTokens || 0,
        charactersProcessed: 0, // 간단화
        messageCount: engine11Data?.messageCount || 0,
        lastUsedAt: engine11Data?.lastUsedAt,
        limits: PLAN_LIMITS[user.plan]?.["11"] || PLAN_LIMITS.free["11"],
      },
      "22": {
        monthlyTokensUsed: engine22Data?.totalTokens || 0,
        inputTokens: engine22Data?.inputTokens || 0,
        outputTokens: engine22Data?.outputTokens || 0,
        charactersProcessed: 0, // 간단화
        messageCount: engine22Data?.messageCount || 0,
        lastUsedAt: engine22Data?.lastUsedAt,
