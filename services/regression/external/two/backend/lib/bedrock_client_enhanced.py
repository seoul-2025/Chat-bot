...
CLAUDE_MODEL_ID = BEDROCK_CONFIG.get('model_id', 'us.anthropic.claude-opus-4-1-20250805-v1:0')
MAX_TOKENS = BEDROCK_CONFIG.get('max_tokens', 16384)
TEMPERATURE = BEDROCK_CONFIG.get('temperature', 0.81)   # 균형잡힌 창의성
TOP_P = BEDROCK_CONFIG.get('top_p', 0.9)
TOP_K = BEDROCK_CONFIG.get('top_k', 50)
...
class BedrockClientEnhanced:
    ...
    def stream_bedrock(
        ...
        try:
            ...
            logger.info(f"System prompt (static): {len(system_prompt)} chars")
            logger.info(f"User message (with context): {len(enhanced_user_message)} chars")

            # Claude 스트리밍 응답 생성 (캐싱 활성화)
            for chunk in stream_claude_response_enhanced(
                user_message=enhanced_user_message,  # 대화 컨텍스트가 포함된 메시지
                system_prompt=system_prompt,  # 정적 시스템 프롬프트
