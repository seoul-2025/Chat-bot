...
conversations_table = dynamodb.Table(os.environ.get('CONVERSATIONS_TABLE', 'sedaily-column-conversations'))

class ConversationManager:
    """대화 내역을 DynamoDB에서 관리"""
    
    @staticmethod
    def save_message(conversation_id: str, role: str, content: str, engine_type: str = 'C1', user_id: str = None):
        """개별 메시지 저장"""
        try:
    ...
    @staticmethod
    def create_or_update_conversation(conversation_id: str, engine_type: str = 'C1', title: str = None):
        """대화 생성 또는 메타데이터 업데이트"""
        try:
            timestamp = datetime.utcnow().isoformat() + 'Z'
