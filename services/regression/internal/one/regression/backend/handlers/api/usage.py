...
usage_table = dynamodb.Table(os.environ.get('USAGE_TABLE', 'sedaily-column-usage'))
...
def handler(event, context):
    ...
    try:
        ...
        if authorizer_context:
            ...
            user_plan = authorizer_context.get('plan', 'free')
            logger.info(f"Multi-tenant mode: tenant={tenant_id}, user={auth_user_id}, role={user_role}, plan={user_plan}")
        
        # OPTIONS 요청은 이미 처리됨 (상단에서)
        
        # Body 파싱 (Base64 디코딩 포함)
        body = event.get('body')
        ...
        if http_method == 'GET':
            # 멀티테넌트: Authorizer에서 userId 가져오거나 path params 사용
            if auth_user_id:
                ...
                logger.info(f"Using userId from path params: {user_id}")
            ...
            # 멀티테넌트: Authorizer에서 플랜 정보 사용 또는 기본값
            if user_plan:
                logger.info(f"Using plan from authorizer: {user_plan}")
            else:
                user_plan = data.get('userPlan', 'free')
                logger.info(f"Using plan from body or default: {user_plan}")
