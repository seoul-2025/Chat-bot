...
const Dashboard = ({ selectedEngine = "C1", onBack }) => {
  ...
  const loadUsageData = async (forceRefresh = false) => {
    try {
      ...
      setUsageData({
        ...
        userPlan: 'free',
        C1: { monthlyTokensUsed: 0, inputTokens: 0, outputTokens: 0, charactersProcessed: 0 },
        C2: { monthlyTokensUsed: 0, inputTokens: 0, outputTokens: 0, charactersProcessed: 0 }
      });
    }
  };
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div className="flex gap-1 sm:gap-2 bg-bg-300/50 rounded-lg p-0.5 sm:p-1">
                {[
                  { id: 'C1', label: 'C1', fullLabel: '칼럼 기본 엔진', path: '/t5/dashboard' },
                  { id: 'C2', label: 'C2', fullLabel: '칼럼 고급 엔진', path: '/c7/dashboard' }
                ].map((engine) => (
                  <button
                    key={engine.id}
